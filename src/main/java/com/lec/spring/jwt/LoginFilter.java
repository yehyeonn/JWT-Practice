...
public class LoginFilter extends UsernamePasswordAuthenticationFilter { // filter 객체를 상속해 수정
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        ...
        Authentication token = new UsernamePasswordAuthenticationToken(username.toUpperCase(), password, null); // null: 권한
    ...
    // 로그인(인증) 성공 시 실행되는 메소드(여기서 JWT 를 발급하자)
    @Override
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        // 라이브러리 활용! server 창 security key 필요(yml + Util)
